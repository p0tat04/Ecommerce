...
if (isset($_POST['place_order']) && !empty($order_items)) {
    ...
    $item_count = array_sum(array_column($order_items, 'qty'));
    $order_item_names = array_map(fn($item) => $item['name'] . ' ×' . $item['qty'], $order_items);
    $order = [
        'id' => $order_id,
        'items' => $item_count . ' item' . ($item_count !== 1 ? 's' : ''),
        'items_list' => $order_item_names,
        'total' => '₱' . number_format($total),
        'status' => 'Processing',
        'badge' => 'badge-info',
        'date' => date('M j, Y'),
    ];
    array_unshift($_SESSION['recent_orders'], $order);
    if (count($_SESSION['recent_orders']) > 5) {
        $_SESSION['recent_orders'] = array_slice($_SESSION['recent_orders'], 0, 5);
    }
    ...
    <?php if ($order_placed): ?>
      ...
      <div class="card">
        <div class="card-body order-success">
          ...
          <p class="success-sub">Your order <strong><?= htmlspecialchars($current_order_id ?? ('#ORD-' . mt_rand(1000, 9999))) ?></strong> has been confirmed. A confirmation email has been sent to <strong><?= htmlspecialchars(auth_user_email() ?? 'your email') ?></strong>.</p>
